...
const templates = [
  { id: "business", name: "Modern Business", desc: "Corporate blue-gradient design" },
  { id: "local", name: "Local Service", desc: "Clean layout highlighting location" },
  { id: "tutor", name: "Tutor & Education", desc: "Friendly scholastic aesthetic" },
  { id: "freelancer", name: "Freelancer Portfolio", desc: "High-tech clean dark theme" },
  { id: "dark", name: "Premium Dark", desc: "Luxe gold-bordered dark style" },
  { id: "whatsapp", name: "WhatsApp Status", desc: "Optimized 9:16 vertical layout" },
  { id: "instaStory", name: "Instagram Story", desc: "Vibrant story layout (9:16)" },
  { id: "instaPost", name: "Instagram Post", desc: "Square layout (1:1)" },
  { id: "flyer", name: "Print Flyer", desc: "Classic structured flyer (A4)" },
  { id: "minimal", name: "Minimal Professional", desc: "Ultra-clean serif design" }
];
...
export default function PosterGeneratorPage() {
  ...
  // Export states
  const [exportingFormat, setExportingFormat] = useState<"png" | "jpg" | "pdf" | "share" | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadServiceData() {
      try {
        ...
        setError("Failed to load service details. Make sure you own this listing.");
      } finally {
        setLoading(false);
      }
    }
  ...
  const exportPoster = async (format: "png" | "jpg" | "pdf" | "share") => {
    ...
    try {
      ...
      const cleanTitle = service.title.toLowerCase().replace(/[^a-z0-9]+/g, "-");
      ...
      if (format === "share") {
        canvas.toBlob(async (blob) => {
          ...
          const file = new File([blob], `${cleanTitle}-poster.png`, { type: "image/png" });
          if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
            ...
            link.click();
            setExportSuccess(true);
            setExportMessage("Sharing not supported natively. Poster downloaded as PNG.");
          }
          setExportingFormat(null);
        }, "image/png");
      } else if (format === "pdf") {
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm font-semibold text-slate-500 mt-3">Loading poster generator...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh] p-4">
        <div className="bg-white p-6 border border-slate-200 rounded-2xl shadow-xl max-w-md w-full text-center space-y-3">
          ...
          <h2 className="text-lg font-extrabold text-slate-800">Poster Error</h2>
          <p className="text-sm text-slate-500">{error || "Service listing not found."}</p>
          <button
    ...
    <div className="space-y-6 pb-20 max-w-[1400px] mx-auto font-sans">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-extrabold text-slate-800">Premium Ad Poster Generator</h2>
            <p className="text-xs text-slate-500 mt-0.5">Generate marketing flyers for social media</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-6 order-2 lg:order-1">
            ...
            <div className="grid grid-cols-2 gap-2">
              {templates.map((t) => (
                ...
                >
                  ...
                  <span className="block text-[9px] text-slate-400 font-medium mt-0.5 leading-tight">{t.desc}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {/* 3. Typography select */}
          <div className="space-y-2">
            <label htmlFor="typography-select" className="block text-xs font-extrabold text-slate-500 uppercase tracking-wider">
          ...
          <div className="space-y-2">
            <label htmlFor="cta-select" className="block text-xs font-extrabold text-slate-500 uppercase tracking-wider">
        ...
        <div className="lg:col-span-7 space-y-6 order-1 lg:order-2">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              >
                {exportingFormat === "png" ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileImage className="h-4 w-4" />}
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-1.5 py-3 px-4 bg-blue-600 hover:bg-blue-700 text-white text-xs font-extrabold rounded-2xl shadow-sm transition active:scale-95 disabled:opacity-50 cursor-pointer transition-colors duration-150"
              >
                {exportingFormat === "jpg" ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileImage className="h-4 w-4" />}
              ...
              >
                {exportingFormat === "share" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Share2 className="h-4 w-4" />}
