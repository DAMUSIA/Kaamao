...
export default function PosterTemplate({
  ...
  const mainContact = contactNumbers.length > 0 ? contactNumbers[0] : "Call Provider";
  ...
  switch (templateId) {
    ...
    case "business":
      return (
        <div className={`w-full h-full bg-gradient-to-br from-slate-900 via-slate-800 to-blue-950 text-white p-6 flex flex-col justify-between relative overflow-hidden ${getFontClass()}`}>
          ...
          <div className="flex items-center justify-between border-b border-white/10 pb-3 z-10 shrink-0">
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-extrabold text-xs uppercase tracking-wider">Kaamao Connect</span>
            </div>
          ...
          <div className="my-auto py-2 space-y-4 text-center z-10">
            <span className="text-[10px] font-extrabold uppercase text-blue-400 tracking-widest">Premium Service</span>
            <h2 className="text-xl sm:text-2xl font-black leading-tight line-clamp-2">{title}</h2>
            <p className="text-[11px] text-slate-300 line-clamp-3 font-medium max-w-xs mx-auto leading-relaxed">{description}</p>
            
            {/* Price Badge */}
            {startingPrice && (
              <div className="inline-flex items-center gap-1 bg-white/5 border border-white/10 px-3.5 py-1 rounded-xl mx-auto shadow-sm">
                <span className="text-[9px] font-bold text-slate-400 uppercase">Starts at</span>
                <span className="text-sm font-black flex items-center text-blue-400">
                  <IndianRupee className="h-3.5 w-3.5" />
                  {startingPrice}
                  {priceUnit && <span className="text-[10px] font-normal text-slate-400"> / {priceUnit.toLowerCase()}</span>}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-3 flex items-center justify-between gap-4 z-10 shrink-0">
            <div className="text-left space-y-1">
              <span className="text-[8px] font-bold uppercase tracking-wider text-slate-400">Contact Partner</span>
              <span className="block text-xs font-black">{mainContact}</span>
              {secondaryContact && <span className="block text-[10px] text-slate-350 font-bold">{secondaryContact}</span>}
            ...
            <div className="bg-white p-1 rounded-lg shrink-0">
              ...
              <img src={qrCodeUrl} alt="QR Code" crossOrigin="anonymous" className="w-14 h-14 bg-white" />
            </div>
          </div>
          ...
          <div className="text-center text-[9px] text-slate-500 font-bold border-t border-white/5 pt-2 z-10 flex justify-between shrink-0">
            ...
            <span className="text-blue-400 uppercase tracking-widest">{ctaText}</span>
          </div>
        </div>
      );
    ...
    case "local":
      return (
        <div className={`w-full h-full bg-slate-50 text-slate-800 p-6 flex flex-col justify-between border border-slate-200 ${getFontClass()}`}>
          ...
          <div className="my-auto py-2 space-y-3.5">
            <h2 className="text-lg sm:text-xl font-extrabold text-slate-900 leading-snug">{title}</h2>
            ...
            <p className="text-[11px] text-slate-500 line-clamp-3 leading-relaxed font-medium">{description}</p>
            ...
            {startingPrice && (
              <div className="flex items-baseline gap-1 text-slate-900">
                ...
                <span className="text-lg font-black text-blue-650 flex items-center">
                  ...
                  {startingPrice}
                  {priceUnit && <span className="text-xs font-normal text-slate-550"> / {priceUnit.toLowerCase()}</span>}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="border-t border-slate-200 pt-4 flex items-center justify-between gap-4 shrink-0">
            <div className="space-y-1">
              <span className="text-[9px] font-bold uppercase tracking-wider text-slate-450">Contact Us Now</span>
              <span className="block text-sm font-black text-slate-900">{mainContact}</span>
            ...
            <div className="text-center">
              ...
              <span className="text-[7px] font-bold text-slate-400 block mt-1">Scan to Call</span>
            </div>
          </div>
        </div>
      );
    ...
    case "tutor":
      return (
        <div className={`w-full h-full bg-amber-50 text-slate-800 p-6 flex flex-col justify-between border-2 border-amber-200 relative overflow-hidden ${getFontClass()}`}>
          {/* Top category ribbon */}
          ...
          <div className="space-y-1 text-left shrink-0">
            <span className="text-[9px] font-extrabold text-amber-600 tracking-wider block">LEARN WITH THE BEST</span>
            <h4 className="text-base font-extrabold text-slate-900 leading-snug">{providerName}</h4>
          </div>

          <div className="my-auto py-2 space-y-3">
            <h2 className="text-lg sm:text-xl font-black text-slate-900 tracking-tight leading-tight">{title}</h2>
            ...
            <div className="flex flex-wrap gap-1.5 text-[10px] font-bold text-slate-500">
              <span className="px-2 py-0.5 bg-amber-100 rounded">📍 {location}</span>
              <span className="px-2 py-0.5 bg-amber-100 rounded">⭐ {ratingAverage.toFixed(1)} score</span>
            </div>
          </div>

          <div className="bg-amber-100 border border-amber-200 rounded-2xl p-3 flex items-center justify-between gap-4 shrink-0">
            <div className="text-left space-y-1">
              <span className="text-[9px] font-extrabold text-amber-600 uppercase tracking-widest">Pricing & Contact</span>
              {startingPrice ? (
                <div className="flex items-center text-sm font-black text-slate-850">
                  <IndianRupee className="h-3.5 w-3.5" />
                  {startingPrice}
                  {priceUnit && <span className="text-[9px] font-normal text-slate-500">/{priceUnit.toLowerCase()}</span>}
                </div>
              ) : (
                <span className="block text-xs font-bold text-slate-600">On Enquiry</span>
              )}
              <span className="block text-xs font-extrabold text-slate-900">{mainContact}</span>
            </div>
    ...
    case "freelancer":
      return (
        <div className={`w-full h-full bg-slate-950 text-white p-6 flex flex-col justify-between relative border border-slate-800 ${getFontClass()}`}>
          ...
          <div className="flex items-center justify-between shrink-0">
            <span className="text-[9px] font-bold tracking-wider text-slate-400 uppercase">Freelance Professional</span>
          ...
          <div className="my-auto py-2 space-y-4">
            ...
            <p className="text-[11px] text-slate-400 line-clamp-3 leading-relaxed font-medium">{description}</p>
            
            <div className="flex items-center gap-1.5 text-xs text-slate-300">
              <span className="text-cyan-400 font-extrabold">By {providerName}</span>
          ...
          <div className="border-t border-slate-900 pt-4 flex items-center justify-between gap-4 shrink-0">
            <div className="space-y-1">
              <span className="text-[9px] font-bold text-slate-400 uppercase">Starting at</span>
              <div className="text-base font-black text-cyan-400 flex items-center">
                <IndianRupee className="h-4 w-4" />
                {startingPrice ?? "Quote"}
                {startingPrice && priceUnit && <span className="text-[10px] font-normal text-slate-500"> / {priceUnit.toLowerCase()}</span>}
              </div>
              <span className="block text-xs font-bold text-slate-100">{mainContact}</span>
            </div>

            <div className="bg-slate-900 border border-slate-800 p-1 rounded-xl">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={qrCodeUrl} alt="QR Code" crossOrigin="anonymous" className="w-12 h-12 bg-white" />
            </div>
          </div>
        </div>
      );
    ...
    case "dark":
      return (
        <div className={`w-full h-full bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-white p-6 flex flex-col justify-between border border-amber-500/20 relative overflow-hidden ${getFontClass()}`}>
          {/* Glowing borders */}
          ...
          <div className="flex items-center justify-between border-b border-white/5 pb-3 shrink-0">
            <span className="text-[9px] font-extrabold uppercase tracking-widest text-amber-500">Luxe Service</span>
          ...
          <div className="my-auto py-2 space-y-4 text-center">
            ...
            {startingPrice && (
              <div className="inline-flex items-center gap-1.5 bg-amber-500/10 border border-amber-500/20 px-3.5 py-1.5 rounded-xl">
                <span className="text-[9px] font-bold text-amber-550 uppercase">Investment starts at</span>
            ...
            <p className="text-[11px] text-slate-300 line-clamp-3 leading-relaxed font-medium max-w-xs mx-auto">{description}</p>
          </div>

          {/* Contact and QR */}
          <div className="border-t border-white/5 pt-4 flex items-center justify-between gap-4 shrink-0">
            <div className="text-left space-y-1">
              <span className="text-[8px] font-bold uppercase tracking-wider text-slate-450">Verified Instructor</span>
              <span className="block text-xs font-black text-white">{providerName}</span>
              <span className="block text-xs text-amber-500 font-extrabold">{mainContact}</span>
    ...
    case "whatsapp":
      return (
        <div className={`w-full h-full bg-gradient-to-br from-teal-900 via-slate-900 to-slate-950 text-white p-8 flex flex-col justify-between text-center relative overflow-hidden ${getFontClass()}`}>
          ...
          <div className="space-y-2 shrink-0">
            ...
            <h4 className="text-xs font-bold text-slate-450 tracking-wider block mt-1">AVAILABLE ON DEMAND</h4>
          </div>

          <div className="my-auto py-4 space-y-4">
            <h2 className="text-xl sm:text-2xl font-black tracking-tight leading-tight">{title}</h2>
            <p className="text-xs text-slate-300 leading-relaxed max-w-xs mx-auto line-clamp-4">{description}</p>
            
            {startingPrice && (
              <div className="inline-flex items-center gap-1 bg-white/5 border border-white/10 px-4 py-2 rounded-2xl">
                <span className="text-[10px] font-bold text-slate-400 uppercase">Starts from</span>
          ...
          <div className="space-y-4 shrink-0">
            <div className="bg-white p-2 rounded-2xl inline-block shadow-xl mx-auto">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={qrCodeUrl} alt="QR Code" crossOrigin="anonymous" className="w-18 h-18 bg-white" />
            </div>
            
            <div className="space-y-1">
              <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest">CALL OR MESSAGE</span>
              <span className="block text-base font-black text-teal-400">{mainContact}</span>
              <span className="block text-[10px] text-slate-500 font-medium">📍 {location} • {providerName}</span>
            </div>
    ...
    case "instaStory":
      return (
        <div className={`w-full h-full bg-gradient-to-tr from-indigo-950 via-slate-900 to-purple-950 text-white p-8 flex flex-col justify-between text-center relative overflow-hidden ${getFontClass()}`}>
          ...
          <div className="space-y-2 shrink-0">
            ...
            <span className="text-[10px] font-extrabold uppercase tracking-widest text-indigo-400 block mt-2">Professional Listing</span>
          ...
          <div className="my-auto py-4 space-y-4">
            <h2 className="text-xl sm:text-2xl font-extrabold tracking-tight leading-snug">{title}</h2>
            <p className="text-xs text-slate-300 leading-relaxed font-medium line-clamp-4 max-w-xs mx-auto">{description}</p>
          ...
          <div className="space-y-4 shrink-0">
            <div className="flex justify-center">
              <div className="bg-white p-2 rounded-2xl shadow-xl shadow-black/30 border border-white/10">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={qrCodeUrl} alt="QR Code" crossOrigin="anonymous" className="w-16 h-16 bg-white" />
              </div>
            </div>
            
            <div className="space-y-1">
              <span className="block text-[9px] font-bold text-slate-400 uppercase tracking-widest">Connect details</span>
              <span className="block text-base font-black text-white">{mainContact}</span>
              {startingPrice && (
                <span className="block text-xs font-bold text-indigo-400">Starts at ₹{startingPrice}/{priceUnit || "hour"}</span>
              )}
            </div>
    ...
    case "instaPost":
      return (
        <div className={`w-full h-full bg-slate-900 text-white p-6 flex flex-col justify-between border-2 border-blue-500/30 relative overflow-hidden ${getFontClass()}`}>
          ...
          <div className="my-auto py-3 space-y-3.5">
            <h2 className="text-lg sm:text-xl font-black leading-tight tracking-tight text-white">{title}</h2>
            <p className="text-[11px] text-slate-350 line-clamp-3 leading-relaxed font-medium">{description}</p>
            ...
            <div className="flex flex-wrap items-center gap-3 text-[10px] font-bold text-slate-400">
              ...
              <span>📍 {location}</span>
              <span className="w-1 h-1 bg-slate-700 rounded-full" />
              <span className="text-amber-400 flex items-center gap-0.5">★ {ratingAverage.toFixed(1)}</span>
            </div>
          </div>
            ...
            <div className="text-left space-y-1">
              {startingPrice ? (
                <div className="space-y-0.5">
                  <span className="text-[8px] font-bold text-slate-450 uppercase block">Starting Price</span>
                ...
                <span className="text-xs font-extrabold text-slate-300">Price on Enquiry</span>
              )}
              <span className="block text-xs font-black text-white pt-0.5">{mainContact}</span>
            </div>
            
            <div className="flex items-center gap-3">
              <div className="bg-white p-1 rounded-xl">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={qrCodeUrl} alt="QR Code" crossOrigin="anonymous" className="w-11 h-11 bg-white" />
              </div>
    ...
    case "flyer":
      return (
        <div className={`w-full h-full bg-white text-slate-800 p-7 flex flex-col justify-between border-4 border-slate-900 relative ${getFontClass()}`}>
          <div className="absolute top-4 left-4 right-4 h-1 bg-slate-950" />
          
          <div className="text-center space-y-1 pt-2 shrink-0">
            <span className="text-[10px] font-extrabold uppercase tracking-widest text-slate-500 block">PUBLIC ANNOUNCEMENT</span>
            <h3 className="text-base font-extrabold text-slate-900 tracking-tight">{category.toUpperCase()} SERVICES</h3>
          </div>
          ...
          <div className="my-auto py-3 space-y-4">
            ...
            <p className="text-[11px] text-slate-650 leading-relaxed font-medium text-center max-w-sm mx-auto">{description}</p>
            
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-center space-y-1">
              <span className="text-[9px] font-extrabold text-slate-500 uppercase tracking-widest">Service Details</span>
              <div className="flex justify-around text-xs font-bold text-slate-800">
                <span>📍 {location}</span>
                <span>⭐ {ratingAverage.toFixed(1)} Avg score</span>
                {startingPrice && (
                  <span className="text-blue-750">₹{startingPrice}/{priceUnit || "hour"}</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="border-t-2 border-slate-950 pt-4 flex items-center justify-between gap-5 shrink-0">
            <div className="text-left space-y-1.5">
              <span className="text-[9px] font-extrabold text-slate-550 uppercase tracking-wider block">CONTACT PROVIDER</span>
              <span className="block text-sm font-extrabold text-slate-950">{providerName}</span>
              <span className="block text-base font-black text-blue-650">{mainContact}</span>
              {secondaryContact && <span className="block text-xs font-semibold text-slate-500">{secondaryContact}</span>}
            </div>
            ...
            <div className="text-center">
              ...
              <span className="text-[8px] font-black text-slate-900 block mt-1">SCAN FOR DETAILS</span>
            </div>
          </div>
    ...
    default:
      return (
        <div className={`w-full h-full bg-white text-slate-800 p-6 flex flex-col justify-between border border-slate-200 ${getFontClass()}`}>
          <div className="flex items-center justify-between border-b border-slate-100 pb-3 shrink-0">
            <span className="text-xs font-black text-slate-900 tracking-wider">{providerName}</span>
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{category}</span>
          </div>

          <div className="my-auto py-2 space-y-3">
            <h2 className="text-lg font-bold text-slate-900 tracking-tight leading-snug">{title}</h2>
            <p className="text-xs text-slate-500 line-clamp-4 leading-relaxed font-medium">{description}</p>
            ...
            <div className="text-left space-y-1">
              {startingPrice && (
                <div className="text-xs font-bold text-slate-650">
                  Starts at <span className="font-extrabold text-slate-900">₹{startingPrice}</span> /{priceUnit || "hour"}
                </div>
              )}
              <span className="block text-xs font-extrabold text-slate-850">Contact: {mainContact}</span>
            </div>
